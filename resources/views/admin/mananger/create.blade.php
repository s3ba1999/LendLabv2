...
@section('content')
    <div class="container-fluid">
        <div class="d-sm-flex justify-content-between align-items-center mb-4" style="font-weight: bold;">
            <h3 class="text-dark mb-0">Añadir Encargado</h3>
        </div>
        <form method="POST" action="{{route('mananger.store')}}">
            @csrf
            <div class="row">
                <div class="col">
                    <div class="user">
                        ...
                        <div class="mb-3"><select class="form-control form-control-user" type="text" id="career" placeholder="Laboratorio" name="laboratory_id" value="{{ old('laboratory_id') }}" required>@foreach ($labs as $lab)    <option value="{{ $lab->id }}">{{ $lab->name }}</option>@endforeach</select>
                        </div>
                        <div class="row mb-3">
